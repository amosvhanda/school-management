...
class StudentAdmissionService
{
    ...
    public function admitStudent(array $studentData, array $guardianData, int $schoolId): Student
    {
        return DB::transaction(function () use ($studentData, $guardianData, $schoolId) {
            ...
            // Create enrollment
            if (!empty($studentData['class_id']) && !empty($studentData['academic_year'])) {
                Enrollment::create([
     ...
     */
    protected function generateStudentNumber(int $schoolId): string
    {
        $school = School::findOrFail($schoolId);
        $code = strtoupper(substr($school->code, 0, 3));
        $year = date('Y');
